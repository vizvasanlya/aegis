"""Sandbox backend registry — selected via AEGIS_RUNTIME_BACKEND (default: docker)."""
...
async def _docker_backend(
    *,
    ...
    Uses :class:`AegisDockerSandboxClient` to inject NET_ADMIN /
    ...
    would call it too, but Aegis manages session lifetime explicitly via
    ...
    from aegis.runtime.docker_client import AegisDockerSandboxClient

    client = AegisDockerSandboxClient(docker.from_env())
    client.aegis_bind_mounts = bind_mounts or []
...
def get_backend(name: str) -> SandboxBackend:
    ...
    if backend is None:
        ...
        raise ValueError(
            f"Unknown AEGIS_RUNTIME_BACKEND: {name!r} (supported: {supported})",
        )
