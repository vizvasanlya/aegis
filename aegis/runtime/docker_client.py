"""AegisDockerSandboxClient — preserves the image's ENTRYPOINT and adds
...
class AegisDockerSandboxClient(DockerSandboxClient):
    ...
    aegis_bind_mounts: list[dict[str, Any]] = []  # overridden per-instance in backends.py
    ...
    async def _create_container(
        ...
        *,
        ...
        # Aegis delta from the SDK body: drop ``entrypoint`` override and
        ...
        # Aegis injections — append, don't overwrite, so FUSE/SYS_ADMIN survives.
        ...
        # Aegis injection: host bind mounts (e.g. large repos passed via --mount)
        # that bypass the SDK's file-by-file LocalDir copy.
        bind_mounts = getattr(self, "aegis_bind_mounts", ())
        if bind_mounts:
