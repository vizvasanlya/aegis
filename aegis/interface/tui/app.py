...
from aegis.core.hooks import BudgetExceededError
from aegis.core.runner import run_aegis_scan
from aegis.interface.tui.live_view import TuiLiveView
from aegis.interface.tui.messages import send_user_message_to_agent
from aegis.interface.tui.renderers import render_tool_widget
from aegis.interface.tui.renderers.agent_message_renderer import AgentMessageRenderer
from aegis.interface.tui.renderers.user_message_renderer import UserMessageRenderer
from aegis.interface.utils import build_tui_stats_text
from aegis.report.state import ReportState, set_global_report_state
...
def get_package_version() -> str:
    try:
        return pkg_version("aegis-agent")
...
class ChatTextArea(TextArea):  # type: ignore[misc]
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._app_reference: AegisTUIApp | None = None

    def set_app_reference(self, app: "AegisTUIApp") -> None:
...
class SplashScreen(Static):  # type: ignore[misc]
    ...
    def _build_url_text(self) -> Text:
        return Text("aegis.ai", style=Style(color=self.PRIMARY_GREEN, bold=True))

    def _build_welcome_text(self) -> Text:
        text = Text("Welcome to ", style=Style(color="white", bold=True))
        text.append("Aegis", style=Style(color=self.PRIMARY_GREEN, bold=True))
    ...
    def _build_start_line_text(self, phase: int) -> Text:
        full_text = "Starting Aegis Agent"
...
class HelpScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("Aegis Help", id="help_title"),
            Label(
                "F1        Help\nCtrl+Q/C  Quit\nESC       Stop Agent\n"
...
class QuitScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("Quit Aegis?", id="quit_title"),
            Grid(
...
class AegisTUIApp(App):  # type: ignore[misc]
    ...
    def _merge_renderables(renderables: list[Any]) -> Text:
        ...
        for i, item in enumerate(renderables):
            if i > 0:
                combined.append("\n")
            AegisTUIApp._append_renderable(combined, item)
        return AegisTUIApp._sanitize_text(combined)
    ...
    def _append_renderable(combined: Text, item: Any) -> None:
        ...
        if isinstance(item, Text):
                combined.append_text(AegisTUIApp._sanitize_text(item))
        elif isinstance(item, Group):
            for j, sub in enumerate(item.renderables):
                if j > 0:
                    combined.append("\n")
                AegisTUIApp._append_renderable(combined, sub)
        else:
            inner = getattr(item, "content", None) or getattr(item, "renderable", None)
            if inner is not None:
                AegisTUIApp._append_renderable(combined, inner)
            else:
            ...
            try:
                ...
                try:
                    if not self._scan_stop_event.is_set():
                        image = load_settings().runtime.image or "aegis-sandbox:latest"
    ...
    def _clean_copied_text(text: str) -> str:
        ...
        for line in lines:
            ...
            if stripped in AegisTUIApp._DECORATIVE_LINES:
                continue
            if stripped and all(c == "─" for c in stripped):
                continue
            out = line
            for prefix in AegisTUIApp._ICON_PREFIXES:
