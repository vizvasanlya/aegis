...
from aegis.agents.factory import build_aegis_agent, make_child_factory
...
async def run_aegis_scan(
    *,
    ...
    """Run or resume one Aegis scan against a sandbox."""
    if scan_id is None:
    ...
    logger.info(
        "%s Aegis scan %s (image=%s, max_turns=%d, interactive=%s, run_dir=%s)",
    ...
    if not resolved_model:
        raise RuntimeError(
            "No LLM model configured. Set AEGIS_LLM env or pass model= to run_aegis_scan().",
        )
    ...
    from aegis.tools.notes.tools import hydrate_notes_from_disk
    from aegis.tools.todo.tools import hydrate_todos_from_disk
    ...
    try:
        ...
        logger.warning(
            ...
            "Resume with 'aegis --resume %s' once the limit clears.",
            scan_id,
            exc,
            scan_id,
        )
        ...
        logger.exception("Aegis scan %s failed", scan_id)
        if root_id is not None:
            await coordinator.cancel_descendants(root_id)
            with contextlib.suppress(Exception):
        ...
        logger.info("Aegis scan %s done", scan_id)
        teardown_logging()
